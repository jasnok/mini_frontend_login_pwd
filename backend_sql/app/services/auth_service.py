...
def sign_up_process(auth: AuthCreate):
    ...
    try:
        ...
        # 새 사용자 삽입
        cursor.execute(
            "INSERT INTO customers (id, pwd, name) VALUES (%s, %s, %s) RETURNING *",
            (auth.id, hash_password(auth.pwd), auth.name)
        )
        result = cursor.fetchone()
        conn.commit()
        ...
        return AuthPublic.model_validate(dict(result))
...
def update_process(auth: AuthUpdate):
    ...
    try:
        ...
        # 정보 업데이트
        cursor.execute(
            "UPDATE customers SET pwd = %s, name = %s WHERE id = %s RETURNING *",
            (hash_password(auth.pwd), auth.name, auth.id)
        )
        result = cursor.fetchone()
        conn.commit()
...
def update_password_process(
    ...
    try:
        ...
        cursor.execute(
            "UPDATE customers SET pwd = %s WHERE id = %s RETURNING *",
            (hash_password(password_data.new_pwd), input_id)
        )
        result = cursor.fetchone()
        conn.commit()
